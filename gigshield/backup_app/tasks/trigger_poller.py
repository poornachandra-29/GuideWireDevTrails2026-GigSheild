...
def poll_all_cities():
    ...
    db = SessionLocal()
    try:
        # Get unique cities with active workers
        cities = db.query(Worker.city).filter(Worker.status == StatusType.active).distinct().all()
        ...
        for city in cities:
            try:
                ...
                print(f"[{city}] Temp: {temp}°C, Wind: {wind_kmh:.1f}km/h, Rain: {rain_1h}mm, AQI: {aqi_val}")
